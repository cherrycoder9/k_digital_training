...
function 로컬스토리지불러오기() {
    ...
    if (로컬데이터 == null) {
        제품목록 = [];
        console.log('로컬데이터에 제품이 없다');
    } else {
        제품목록 = 로컬데이터;
        console.log(`로컬데이터 제품목록 ${로컬데이터.length}개 있음`, 로컬데이터);
        console.log(`JS메모리 제품목록에 ${제품목록.length}개 저장됨`, 제품목록);
    }
    // # 0-1. 문제 없음 
...
function 자동주문() {
    ...
    출력인덱스 = 현재불러온제품개수 + Math.floor(Math.random() * 로컬에있는제품개수) - 현재불러온제품개수;
    console.log('출력인덱스', 출력인덱스);
    setInterval(
        () => { 화면갱신(); 현재불러온제품개수++; }
        , 1000);
...
function 화면갱신() {
    ...
    // 3-1. 로컬 스토리지에 저장되어 있는 제품목록을 가져온다 
    로컬스토리지불러오기();
    // # 3-1. 이상 없음

    // 3-2. 제품목록에 있는 배열들을 돌면서 화면을 갱신한다
