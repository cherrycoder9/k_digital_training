...
let movie = 영화API.boxOfficeResult.weeklyBoxOfficeList;
// console.log(movie[0].rank);

for (let i = 0; i < movie.length; i++) {
    html += `
        <tr>
            <th>${movie[i].rank}</th>
            <th>${movie[i].movieNm}</th>
            <th>${movie[i].openDt}</th>
            <th>${movie[i].audiAcc}</th>
            <th>${movie[i].salesAcc}</th>
        </tr>`.trim();
}
document.querySelector('#movieRanking').innerHTML = html;
